...
- **Fullscreen Dialog Scrolling Fix (Aug 10, 2025)**: Resolved critical scrolling issues in logistics management fullscreen view by restructuring flexbox hierarchy. Fixed header now stays in place with `flex-shrink-0`, Tabs component takes `flex-1 flex flex-col overflow-hidden`, and each TabsContent uses `flex-1 overflow-y-auto` for proper scrolling behavior. Also fixed QR scanner modal props mismatch (`isOpen/onClose` → `open/onOpenChange`) to restore button functionality.
