...
- **Authentication System Implementation (Aug 12, 2025)**: Implemented comprehensive NextAuth authentication system with Google SSO as primary method and magic links as backup. Created custom sign-in and error pages with PopCycle branding, implemented authentication middleware to protect portal routes, and updated Navigation component with session-aware AuthButton. Super admin user (terrencestasse@gmail.com) configured with full portal access. System supports capability-based access control where regular users get standard maker accounts while staff roles grant additional dashboard access. **CRITICAL FIX NEEDED**: Google OAuth flow works perfectly but MongoDB user lacks permissions for NextAuth collections (accounts, sessions, verification_tokens) - database permissions must be updated for full authentication functionality.
- **Component Folder Consolidation (Aug 12, 2025)**: Resolved build errors caused by duplicate component folders by consolidating everything into `app/components/` folder following App Router conventions. Removed root-level `components/` folder and updated all import paths. This fixed module resolution conflicts and "Failed to read source code" errors that were preventing proper compilation. All UI components, AuthButton, Navigation, and other components now properly located in single `app/components/` directory structure.
